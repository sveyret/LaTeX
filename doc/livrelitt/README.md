...
Pour débuter un nouveau livre, n'hésitez pas à vous inspirer de [l'exemple livré](exemple).
...
* \pageillustr{_image_} pour ajouter une illustration pleine page (l'illustration sera placée sur l'intégralité de la prochaine page) ; pour plus d'informations sur les formats d'image, voir [ci-dessous](#images).
...
Un fichier _Makefile_ est fourni à titre de modèle pour votre projet de livre. Il se trouve, avec un exemple, dans le répertoire [exemple](exemple). Ce fichier doit être copié dans le répertoire contenant les sources du projet, puis adapté en modifiant les variables qui se trouvent au début du fichier :
